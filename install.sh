...
ok "完成！"
echo ""
echo "  面板端口 : ${PORT}    访问路径要带 /login"
echo "             首次打开是「创建管理员账号」，第一个注册的就是管理员"
echo "             （根路径 / 是探针伪装页，默认没开，只会显示「探针暂时无法访问」）"
echo ""
echo "  ‼️ 重要：必须用 HTTPS 或 localhost 打开，直接用 http://IP:${PORT} 会打不开！"
echo "     面板的加密通道要浏览器的 WebCrypto，而浏览器只在 HTTPS / localhost 下才提供，"
echo "     用裸 IP + http 访问会「全新安装却显示登录页、而且登不进去」。"
echo ""
echo "     最快：在你自己电脑上开隧道，然后浏览器访问 http://127.0.0.1:${PORT}/login"
echo "         ssh -L ${PORT}:127.0.0.1:${PORT} root@<本机IP>"
echo ""
echo "     长期：域名 + 反代 HTTPS（Caddy 两行配置自动签证书），之后 https://域名/login"
